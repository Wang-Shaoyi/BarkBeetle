...
{
    internal class SkeletonPackageManager
    {
        private SkeletonPackage skeletonPackage;
        ...
        public void SetGeometryPackage(double stripWidth, Surface surface, GH_Structure<GH_Point> pointsTree, string so )
        {
            ...
            //Setup skeleton package, different skeletons has different outcomes here
            skeletonPackage = new SkeletonPackage(stripWidth, extendedSurface, closestPtTree, so);

            //setup skeleton crv (because we want to reference the surface so we do it here)
            skeletonPackage.Skeleton.SkeletonCurve = ProcessSkeletonCurve();
            ...
            skeletonPackage.UVCurves = uvCurves;
            skeletonPackage.Skeleton.UVVectors = uvVectors;
        }
        ...
        {
            List<GH_Point> skeletonPoints = skeletonPackage.Skeleton.SkeletonPoints;
            Surface surface = skeletonPackage.ExtendedSurface;
        ...
        {
            Curve skeletonCrv = skeletonPackage.Skeleton.SkeletonCurve.Value;
            GH_Structure<GH_Point> organizedPtsTree = skeletonPackage.OrganizedPtsTree;
